    ...
    <div className="pt-24 pb-12 px-6 max-w-7xl mx-auto min-h-screen relative z-10">
      ...
      >
        {filteredOperativos.map((op: any, idx) => {
          ...
          return (
            <Link key={op.id} to={`/operativos/${op.id}`}>
              <motion.div 
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: idx * 0.05 }}
                whileHover={{ y: -5, scale: 1.02 }}
                className="bg-gradient-to-t from-[#0a0a0a] to-[#111] border border-gray-800 group relative overflow-hidden cursor-pointer h-full"
              >
                <div className="absolute inset-0 bg-blood-red/0 group-hover:bg-blood-red/10 transition-colors z-0" />
                
                <div className="aspect-[3/4] relative z-10 p-2 flex-1">
                  <img 
                    src={localImage} 
                    alt={op.name} 
